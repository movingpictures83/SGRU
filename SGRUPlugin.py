...
class SGRUPlugin:
   ...
   def run(self):
    ...
    # GRU PIPELINE
    model = create_GRU_model('Sequential', int(self.parameters["units"]), train_X, train_y)
    train_GRU(model, float(self.parameters["lr"]), int(self.parameters["epochs"]), train_X, train_y, test_X, test_y)
